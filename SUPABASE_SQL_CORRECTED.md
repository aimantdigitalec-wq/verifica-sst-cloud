# SQL Corregido para Supabase - Verifica SST Cloud

Copia y pega este SQL en Supabase SQL Editor. Este script está corregido para evitar el error de sintaxis.
...
## Paso Siguiente: Crear Políticas RLS

Después de ejecutar el SQL anterior, copia y pega esto en una **NUEVA QUERY**:

```sql
-- Políticas RLS para empresas
DROP POLICY IF EXISTS "Empresas: usuarios pueden ver su propia empresa" ON empresas;
...
DROP POLICY IF EXISTS "Empresas: solo admin puede crear" ON empresas;
...
DROP POLICY IF EXISTS "Verificaciones: usuarios pueden ver de su empresa" ON verificaciones;
...
DROP POLICY IF EXISTS "Verificaciones: usuarios pueden crear" ON verificaciones;
...
DROP POLICY IF EXISTS "Verificaciones: usuarios pueden actualizar" ON verificaciones;
...
DROP POLICY IF EXISTS "Evidencias: usuarios pueden ver de su empresa" ON evidencias;
...
DROP POLICY IF EXISTS "Evidencias: usuarios pueden crear" ON evidencias;
...
DROP POLICY IF EXISTS "Informes: usuarios pueden ver de su empresa" ON informes;
...
DROP POLICY IF EXISTS "Suscripciones: usuarios pueden ver de su empresa" ON suscripciones;
...
DROP POLICY IF EXISTS "Mensajes: usuarios pueden ver sus propios mensajes" ON mensajes_chat;
CREATE POLICY "Mensajes: usuarios pueden ver sus propios mensajes"
  ON mensajes_chat FOR SELECT
  USING (usuario_id = auth.uid());

DROP POLICY IF EXISTS "Mensajes: usuarios pueden crear" ON mensajes_chat;
...
1. **Primera Query:** Copia y pega el primer bloque SQL (tablas e índices) en Supabase SQL Editor
2. Haz clic en **Run**
3. Espera a que se complete
4. **Segunda Query:** Haz clic en **New Query** y copia el segundo bloque (políticas RLS)
5. Haz clic en **Run**
6. ¡Listo! Todas las tablas y políticas están configuradas

## Verificación

Para verificar que todo está correcto, ejecuta esta query:
